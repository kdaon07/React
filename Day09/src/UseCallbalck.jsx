...
export default function UseCallback() {
    ...
    const [users, setUsers] = useState([ // selected항목 추가 토글 속성
        {
            id: 1,
            name: '강민지',
    ...
    const onToggle = useCallback((id, user) => { //1)이름 클릭하면 selected속성 반전 2) input에 현재값 반영하기
        setUsers(
    ...
    const onModify = (user, id) => { //1)수정 버튼 누르면 input의 값으로 user수정하기 2) input clear
    ...
    const countSelected = (users) => users.filter((user) => user.selected == true ).length;
    const sCount = useMemo(() => countSelected(users),[users]);
    return (
        <div> 
